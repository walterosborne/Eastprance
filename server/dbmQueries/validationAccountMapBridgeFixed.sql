/*
Fix for validationAccountMapBridge.sql after ACCT_ID mapped zero accounts.
The validation table stores account IDs as numeric-looking text (for example 4300591.0),
so normalize text -> DECIMAL -> BIGINT before joining to fresh RACCT.

Run this whole file. Send Query A, Query B, Query C, and Query D.
*/
SET NOCOUNT ON;
...
/* Build account mapping dictionary from validation table. */
SELECT
    TRY_CONVERT(BIGINT, TRY_CONVERT(DECIMAL(38,6), NULLIF(LTRIM(RTRIM(CONVERT(varchar(100), V.ACCT_ID))), ''))) AS COST_ELEMENT,
...
WHERE TRY_CONVERT(BIGINT, TRY_CONVERT(DECIMAL(38,6), NULLIF(LTRIM(RTRIM(CONVERT(varchar(100), V.ACCT_ID))), ''))) IS NOT NULL
  AND LEFT(LTRIM(RTRIM(V.Period)), 4) = '2026'
GROUP BY
    TRY_CONVERT(BIGINT, TRY_CONVERT(DECIMAL(38,6), NULLIF(LTRIM(RTRIM(CONVERT(varchar(100), V.ACCT_ID))), ''))),
...
/* QUERY A — mapping dictionary health. Send this one row. */
SELECT
...
/* Build fresh Q1 2026 DS transactions. */
...
LEFT JOIN #AccountMap M
  ON M.COST_ELEMENT = TRY_CONVERT(BIGINT, TRY_CONVERT(DECIMAL(38,6), NULLIF(LTRIM(RTRIM(CONVERT(varchar(100), T.RACCT))), '')))
...
/* QUERY B — mapping coverage by division. Send all four rows. */
SELECT
    DIVISION,
...
/* Apply facility-report category bridge. */
SELECT
    CASE
        WHEN COST_SUMMARY_CATEGORY = 'Leased Land and Buildings' THEN '2 Rent - Land & Building'
        WHEN COST_SUMMARY_CATEGORY LIKE 'Deprec%' OR COST_SUMMARY_CATEGORY LIKE '%LHI%' THEN '3 Depreciation & LHI'
        WHEN COST_SUMMARY_CATEGORY = 'Utilities' THEN '8 Utilities'
        WHEN COST_SUMMARY_CATEGORY LIKE 'Facility Repair & Maint%' THEN '6 Maintenance & Repairs'
        WHEN COST_SUMMARY_CATEGORY IN ('Enterprise Facility Services','Facility Purchased Serv','Facility Purchased Services')
          OR COST_SUMMARY_CATEGORY LIKE 'Facility Purchased Serv%' THEN '7 Services'
        WHEN COST_SUMMARY_CATEGORY = 'Taxes and Insurance' THEN '4+5 Property Taxes & Insurance'
        ELSE NULL
...
/* QUERY C — category x division. Send all rows. */
SELECT
    BRIDGE_CATEGORY,
    DIVISION,
    COUNT(DISTINCT COST_CENTER) AS COST_CENTER_COUNT,
